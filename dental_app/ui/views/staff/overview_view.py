"""Staff overview tab — stat cards, recent activity, quick actions."""
import logging
...
class StaffOverviewView(ttk.Frame):
    ...
    def _build(self):
        ...
        try:
            ...
            create_stat_card(stats_frame, "📅 Today's Appointments",
                             self.app.appointments.count_today(), COLORS["info"], 0)
            create_stat_card(stats_frame, "⏳ Pending Appointments",
                             self.app.appointments.count_by_status("pending"),
                             COLORS["warning"], 1)
            create_stat_card(stats_frame, "✅ Confirmed Appointments",
                             self.app.appointments.count_by_status("confirmed"),
                             COLORS["success"], 2)
            create_stat_card(stats_frame, "🏥 Completed Appointments",
                             self.app.appointments.count_by_status("completed"),
                             COLORS["completed"], 3)
        ...
        ttk.Button(actions_grid, text="📋 Manage Appointments",
