...
class AppointmentManagementMixin:
    def setup_appointments_view(self):
        ...
        # New horizontal container frame for 80/20 split
        horizontal_frame = ttk.Frame(self.appointments_frame)
        horizontal_frame.pack(fill=BOTH, expand=YES, padx=5, pady=5)

        # Left frame (80%) for appointments
        ...
        column_widths = {
            ...
            'service': 180,  # Reduced width
            'date': 90,  # Reduced width
            'time': 90,  # Reduced width
            'contact': 100,  # Reduced width
            'status': 90  # Reduced width
        }
    ...
    def open_appointment_action_modal(self, event):
        ...
        appt_id = self.appt_tree.item(selected_item[0])['values'][0]
    ...
    def edit_appointment(self):
        ...
        appt_id = appt_values[0]
        username = appt_values[1]
        current_service = appt_values[2]
        current_date = appt_values[3]
        current_time = appt_values[4]
        current_status = appt_values[6]
        ...
        service_combo = ttk.Combobox(form_frame, textvariable=service_var,
                                   values=["Cleaning", "Extraction", "Filling", "Braces Consultation", "Other"],
                                   state="readonly", width=30)
        service_combo.grid(row=0, column=1, pady=5)
        ...
        ttk.Label(form_frame, text="(YYYY-MM-DD)", font=("Helvetica", 8)).grid(row=1, column=2, sticky=W)
            ...
            try:
                ...
                if new_date != current_date or new_time != current_time:
                    query = """
                        ...
                        AND id != %s
                    """
                    result = self.app.execute_query(query, (new_date, new_time, appt_id))
                    if result[0][0] > 0:
                ...
                success = self.app.execute_query(
                    ...
                    WHERE id = %s
                    """,
                    (new_service, new_date, new_time, new_status, appt_id),
                    fetch=False
                )
        ...
        ttk.Button(button_frame, text="Save", command=save_changes, bootstyle="success").pack(side=LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=dialog.destroy, bootstyle="secondary").pack(side=LEFT, padx=5)

        # Calculate dialog size based on content
        dialog.update_idletasks()
        width = max(450, dialog.winfo_width())  # Minimum width of 450
        height = max(350, dialog.winfo_height())  # Minimum height of 350
    ...
    def delete_appointment(self):
        ...
        message = f"Are you sure you want to delete this appointment?\n\n" \
                 f"Client: {username}\n" \
                 f"Service: {service}\n" \
                 f"Date: {date}\n" \
                 f"Time: {time}\n\n" \
        ...
        dialog = ModernConfirmationDialog(self, title, message, "danger")
        if not dialog.show():
            return
